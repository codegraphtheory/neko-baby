...
Hermes CLI skins do not set the terminal emulator font directly. The profile ships `scripts/install_launcher.sh` and `scripts/neko-baby` for macOS Terminal.app. The launcher applies `Comic Mono` and a 79-column compact live view only while the `neko-baby` profile is running, then restores the previous Terminal.app font and terminal size. Comic Mono is monospaced, so terminal columns stay aligned and the pet pane appears in the initial live view. Other terminal emulators still need manual font selection.
